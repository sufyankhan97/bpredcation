...
void MCSimulation(double *&records, const uint32_t &size)
{
    ...
    for (int n = 0; n < nphotons; ++n) {
        ...
        while (w != 0) {
            //prajoytg :: NO RAND :: double s = -log(drand48()) / sigma_t;
            //prajoytg :: NO RAND :: double distToBoundary = 0;
            //prajoytg :: NO RAND :: if (muz > 0) distToBoundary = (d - z) / muz;
            //prajoytg :: NO RAND :: else if (muz < 0) distToBoundary = -z / muz;
            ...
            //
            // prajyotg :: ROI
            // IF s> dist --> More number of instructions
            if (s > distToBoundary) {
#ifdef ONED
                // compute diffuse reflectance and transmittance
                if (muz > 0) Tt += w; else Rd += w;
#else
                cout << "PB_if: S = " << s << "; DisttoB = " << distToBoundary << endl;
                ...
                cout << "PB_else: S = " << s << "; DisttoB = " << distToBoundary << endl;
            }
            // [comment]
            // Move photon packet
            // [/comment]
