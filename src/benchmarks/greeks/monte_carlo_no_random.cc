 ...
 * Coursework   : RA, under Prof. Joshua San Miguel
 * Project      : h2p-branches
 * File         : monte_carlo.cc
 * Description  : OOP based implementation of Monte Carlo
 * ********************************************************/

#include "black_scholes.hh"
#include <iostream>
...
void monte_carlo_call_price(const int num_sims, 
  ...
  //cout << "Sp_adjust: " << Sp_adjust << " S_adjust: " << S_adjust << " Sm_adjust: " << Sm_adjust << endl;
  ...
  for (int i=0; i<num_sims; i++) {
    //prajyotg :: gauss_bm is drawn from Gaussian Distribution
    //it gets manipulated and determines if probalistic if is taken
    ...
    // double expgauss = 0.9;  // Precalculate
    double expgauss = 1.2;  // Precalculate
    //cout << "expgauss = " << expgauss << endl;
    Sp_cur = Sp_adjust * expgauss;
    S_cur = S_adjust * expgauss;
    Sm_cur = Sm_adjust * expgauss;
    //cout << "Sp_cur: " << Sp_cur << " S_cur: " << S_cur << " Sm_cur: " << Sm_cur << endl;
...
int main(int argc, char **argv) {
  ...
  //int num_sims = 3;    // Number of simulations to carry out for Monte Carlo    
