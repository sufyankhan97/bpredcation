...
long int mis_pred_count = 0; 
//std::unordered_map<int, int> mispredPC{};
    ...
    while (continue_reading) {
        ...
        if (knob_cloudsuite) 
        {
            ...
            { // successfully read the trace
                ...
                cout << "Current cloudsuite instr pc" << arch_instr.instr_id  << endl;
	    ...
	    else
	    {
	        ...
	        { // successfully read the trace
                ...
                //cout << "arch inst ip=" << hex << arch_instr.ip << endl; 
                //cout << "arch inst id=" << hex << arch_instr.instr_id;
                ...
                for (uint32_t i=0; i<MAX_INSTR_DESTINATIONS; i++) {
                    ...
                    //cout << "arch instr dest reg" << hex << unsigned ( arch_instr.destination_registers[i] ) << endl;
                ...
                */
                ...
                int find_instr_flag = 0; //sufyan

                for (int i=0; i<NUM_INSTR_SOURCES; i++) {
                    arch_instr.source_registers[i] = current_instr.source_registers[i];
                    //cout << "arch instr src reg" << hex << unsigned ( arch_instr.source_registers[i] ) << endl;
                    ...
                    if(arch_instr.source_registers[i] == 87)
                        find_instr_flag = 1;
                    else
                        find_instr_flag = 0;
                    ...
                    if(find_instr_flag)
                        cout << "found exchange instr" << arch_instr.ip << endl;
                ...
                // if(unsigned ( arch_instr.destination_registers[0] ) == unsigned ( arch_instr.source_registers[0] ))
                //     cout << "src reg = dest_reg, ip=" << hex << arch_instr.ip << endl;
                arch_instr.num_reg_ops = num_reg_ops;
                arch_instr.num_mem_ops = num_mem_ops;
                if (num_mem_ops > 0) 
                    arch_instr.is_memory = 1;
                ...
                //code for checking if there is any pre existing instruction with isbranch = 1 and source memory != 0
                ...
                //             flag = 1;
                //         //cout << "src mem" << arch_instr.source_memory[i] << endl;
                //     }
                //     //cout << endl;
                //     if(flag == 0)
                //     {
                //         cout << "ip = " << arch_instr.ip ; 
                //         for(int i = 0 ; i < NUM_INSTR_SOURCES ; i++)
                //         {
                //             cout << arch_instr.source_memory[i] << endl;
                //         }
                //     }
                // }

                //code for finding xchg instr
                if(arch_instr.is_branch == 1)
                {
                    int flag = 0;
                    for(int i = 0 ; i < NUM_INSTR_SOURCES ; i++)
                    {   
                        if(arch_instr.source_memory[i] != 87)
                            flag = 1;
                    }
                    if(flag == 0)
                        cout << "PC for xchg instr" << arch_instr.ip  << endl; 
                }
                // add this instruction to the IFETCH_BUFFER
                if (IFETCH_BUFFER.occupancy < IFETCH_BUFFER.SIZE) 
                {
                    ...
                    if (IFETCH_BUFFER.entry[ifetch_buffer_index].is_branch) 
                    {
                        ...
                        if(IFETCH_BUFFER.entry[ifetch_buffer_index].branch_taken != branch_prediction)
                        {
                            ...
                            {
                                mis_pred_count++;
                                //cout << "Found a misprediction at " << IFETCH_BUFFER.entry[ifetch_buffer_index].ip << "\tmispred = " << mis_pred_count << "\tbranch type = " << (uint32_t)IFETCH_BUFFER.entry[ifetch_buffer_index].branch_type << endl;
                                fetch_stall = 1;
                                instrs_to_read_this_cycle = 0;
                                IFETCH_BUFFER.entry[ifetch_buffer_index].branch_mispredicted = 1;
                                auto it = mispredPC.find(IFETCH_BUFFER.entry[ifetch_buffer_index].ip);
                                if(it != mispredPC.end())
                                {
                                    //found that pc so just increment its count     
                                    //cout << "\tPC present in map, current count = " <<  mispredPC[IFETCH_BUFFER.entry[ifetch_buffer_index].ip] << endl;
                                    mispredPC[IFETCH_BUFFER.entry[ifetch_buffer_index].ip] += 1;    //increment count
                                }
                                else
                                {
                                    //cout << "\tPC not found in map" << endl;
                                    mispredPC[IFETCH_BUFFER.entry[ifetch_buffer_index].ip] = 1;     //new pc so keep count as 1
                                    mispredType[IFETCH_BUFFER.entry[ifetch_buffer_index].ip] = IFETCH_BUFFER.entry[ifetch_buffer_index].branch_type;
                                }
                            }
                        }
                        else
...
uint32_t O3_CPU::add_to_ifetch_buffer(ooo_model_instr *arch_instr)
{
  ...
  */
  ...
  // end magic
  //cout << "INSTR PA1= " << hex << IFETCH_BUFFER.entry[index].instruction_pa << endl;
  //cout << "VIRTUAL PA= " << IFETCH_BUFFER.entry[index].virtual_address << endl;
  ...
  for(uint32_t i=0; i<IFETCH_BUFFER.SIZE; i++)
      ...
      if((IFETCH_BUFFER.entry[index].translated == COMPLETED) && (IFETCH_BUFFER.entry[index].fetched == 0))
	{
	  ...
	  fetch_packet.event_cycle = current_core_cycle[cpu];
        ...
        //cout << "Instr PA=" << fetch_packet.instruction_pa << endl;
	  /*
...
void O3_CPU::print_mispredStats()
{
    for (auto const &pair: mispredPC) {
        std::cout << "{" << pair.first << ": " << pair.second << "}\n";
    }

    for (auto const &pair: mispredType) {
        std::cout << "{" << pair.first << ": " << pair.second << "}\n";
    }
}